...
It was not specifically written to be a single since its creation as a novel was suggest by a publisher's editor after most of the stories had already appeared.
...
It is composed of 26 chapters. Note that the complete version has 28 chapters, but it is not available since two chapters weren't considered for this novel.
...
#First, we want to organize the book in a tibble composed of a tibble with two column: chapter and line row. For having the tidy book, we tokenize, so we have one word per row and then we remove the stop words. At this point, let plot the word frequency
...
# word frequency
# tidy_book %>%
#   filter(word!="it's" || word!="it's" || word!="i'm")  %>%
...
#stemming_word_cors <- word_cors %>%
#  mutate(item1 = wordStem(item1), item2 = wordStem(item2)) # stemming
...
Thus, let's visualize the **correlation plot** of the eight chapter select during this previous phase.
...
The common tie is the insanity. In chapter August 1999: THE EARTH MEN, some martianss became vulnerable since using their telepathy against the humans have an unintended consequence of mental insanity. 
...
most_frequent_sentiment_words <- function(bing_kind, sentimentwords, famous_sentiment_chapter, best_color, normal_color){
  ...
  #print(count_sentiment_words_per_chapter)
...
Let's use LDA is a method for fitting a topic model. It handles each chapter as a mixture of topics, and each topic as a mixture of words. 
...
# ggplot(top_topics_chapters_gamma, aes(x=title, y=gamma, fill=topic)) +
...
<!-- VGhhbmtzIEF1cm9yYSBmb3IgdGhlIGJvb2sgOik= -->
